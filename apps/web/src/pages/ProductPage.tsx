...
export function ProductPage() {
  ...
  // activeThumb: índice de la imagen activa en product.images[]
  // 0 = primera imagen (principal), 1 = segunda, etc.
  const [activeThumb, setActiveThumb] = useState(0)
  const [activeSize,  setActiveSize]  = useState('')
  const [activeColor, setActiveColor] = useState<{ name: string; hex: string } | null>(null)
  const [qty,         setQty]         = useState(1)
  // Si la imagen principal falla al cargar, mostramos el SVG de respaldo
  ...
  if (loading) {
    return (
      <div style={{ padding: '80px var(--pad-x)', textAlign: 'center' }}>
        <p style={{ color: 'var(--gray-400)', fontSize: '13px' }}>Cargando producto...</p>
      </div>
    )
  }
  ...
  const selectedColor = activeColor ?? product.colors[0]
    ...
    // Productos de otras categorías (relleno)
  ...
  const handleAddToCart = () => {
    if (!activeSize) { showToast('Selecciona una talla'); return }
    for (let i = 0; i < qty; i++) {
  ...
  // ── Galería ────────────────────────────────────────────────────────────────
  // Usa product.images si existen, si no crea un array vacío para el fallback
  const images = product.images ?? []

  // Al cambiar de thumb reseteamos el error de imagen
  const handleThumbClick = (i: number) => {
    setActiveThumb(i)
    setImgError(false)   // resetea el error para intentar cargar la nueva imagen
  }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.grid}>
        ...
        <div ref={galleryRef} className={styles.gallery}>
          ...
          {/* Imagen principal — cambia según el thumb activo */}
          <AnimatePresence mode="wait">
            <motion.div
              className={styles.mainImg}
              key={activeThumb}                   // key cambia → dispara animación
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{    opacity: 0 }}
            ...
            >
              ...
              {images[activeThumb] && !imgError ? (
                <img
                  src={images[activeThumb]}       // images[0], images[1], images[2]...
                  alt={`${product.name} - vista ${activeThumb + 1}`}
                  className={styles.mainImgReal}
                  onError={() => setImgError(true)} // si falla → muestra SVG
                />
              ) : (
                // Fallback SVG si no hay imagen o falló al cargar
          ...
          {/* ── Thumbnails ──────────────────────────────────────────────── */}
          {/* Muestra un thumb por cada imagen en product.images            */}
          {/* Si no hay imágenes, muestra 3 thumbs de color de respaldo     */}
          <div className={styles.thumbs}>
            {(images.length > 0 ? images : [null, null, null]).map((img, i) => (
              ...
              >
                {/* Thumb con imagen real */}
                {img ? (
                  <img
                    src={img}
                    alt={`Vista ${i + 1}`}
                    className={styles.thumbImgReal}
                    onError={(e) => {
                      // Si falla, oculta la imagen y muestra el fondo de color
                      (e.target as HTMLImageElement).style.display = 'none'
                    }}
                  />
                ) : (
                  // Thumb de color de respaldo
                  <div className={styles[`thumbBg${(i % 3) + 1}`]} style={{ width: '100%', height: '100%' }} />
                )}
              </div>
            ))}
          </div>
        ...
        <div ref={infoRef} className={styles.info}>
          ...
          <div className={styles.colors}>
            {product.colors.map(color => (
              <button
                key={color.name}
                className={`${styles.colorBtn} ${selectedColor.name === color.name ? styles.colorActive : ''}`}
          ...
          <div className={styles.trust}>
            <div className={styles.trustItem}><HiOutlineTruck size={14} /><span>Envío disponible</span></div>
            <div className={styles.trustItem}><HiOutlineRefresh size={14} /><span>Cambios gratis</span></div>
            <div className={styles.trustItem}><HiOutlineCreditCard size={14} /><span>Pago seguro</span></div>
          </div>
...
// Solo se muestra si no hay imagen o la imagen falló al cargar
